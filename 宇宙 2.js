...
class BubbleGame {
  ...
  showGameOverMessage() {
    ...
    if (!this.nameInput) {
      let playerName = "";
      while (playerName.length === 0 || playerName.length > 6) {
        playerName = prompt("名前を入力してください　※6文字以内:");
        if (playerName.length > 6) {
          alert("名前は6文字以内で入力してください。");
        }
      }
      this.nameInput = playerName || '匿名';
      addScore(this.nameInput, this.score); // スコアをランキングに追加
    }
    ...
    button.addEventListener("click", () => {
      ...
      this.nameInput = undefined; // 名前入力をリセット
    });
...
window.onload = () => {
  ...
  resetButton.addEventListener("click", () => {
    localStorage.removeItem('bubbleGameScores'); // ローカルストレージからランキングを削除
    game.displayRankings(); // ランキング表示をリセット
  });

  // Enterキーの処理
  window.addEventListener("keydown", (e) => {
    if (e.code === "Enter") {
      e.preventDefault(); // デフォルトのEnterキーの挙動をキャンセル
      if (game.nameInput) {
        game.nameInput = undefined; // 名前入力をリセット
        game.init();
        game.displayRankings(); // ランキング表示を更新
      }
  ...
  // Enterキーで名前の入力を終了するための処理
  window.addEventListener("keydown", (e) => {
    if (e.code === "Enter" && game.nameInput) {
      game.nameInput = undefined; // 名前入力をリセット
      game.init();
      game.displayRankings(); // ランキング表示を更新
    }
